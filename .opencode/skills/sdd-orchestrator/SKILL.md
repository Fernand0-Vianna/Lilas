...
description: Conduz o fluxo Spec-Driven Development do Lilás fase a fase — prd.md, design.md, spec.md, tasks.md em specs/features/<feature-id>/ — com versionamento em frontmatter e gate que impede gerar código até aprovação humana de cada fase. Use ao iniciar capability nova, refator grande cujo contrato de negócio/técnico deva ficar documentado, ou quando usuário pedir "orquestrador SDD", "PRD primeiro" ou "spec antes de implementar". Não use para correção de bug, ajuste pequeno dentro de contrato existente, nem para implementar feature cuja spec já está aprovada.
---

# Orquestrador SDD — Lilás

Executor do fluxo SDD. O **governo** (fases, regras, mapeamento) está em [`specs/sdd/SDD-ORCHESTRATOR.md`](../../specs/sdd/SDD-ORCHESTRATOR.md) — documento aprovado e versionado, que esta skill **aplica** em vez de reescrever.
...
1. Gerar **só** o artefato da fase corrente e **parar** para aprovação. Nunca produzir dois artefatos numa passagem.
...
6. **Simetria de domínio:** operações reversíveis (cancelar, despublicar, remover) modeladas com mesmo rigor que construtivas.
...
Se usuário pedir código com spec ainda em Draft: dizer em uma frase que gate está fechado, mostrar o que falta aprovar, e oferecer *spike* descartável como alternativa.
...
| Âmbito | Delegar a | Conhecimento que agente carrega |
| ------ | --------- | ------------------------------- |
| Fronteiras, component hierarchy, roteamento, estado | agent [`frontend-architect`](../../agents/frontend-architect.md) | [`frontend-skill`](../frontend-skill/SKILL.md) |
| Código React (componentes, hooks, services) | agent [`frontend-implementer`](../../agents/frontend-implementer.md) | [`frontend-skill`](../frontend-skill/SKILL.md) |
| Schema, RLS, triggers, RPC | agent [`frontend-implementer`](../../agents/frontend-implementer.md) (com leitura de [`supabase-skill`](../supabase-skill/SKILL.md)) | ambas |
| Testes | agent [`test-engineer`](../../agents/test-engineer.md) | `frontend-skill` / `supabase-skill` conforme camada |
...
2. [ ] Está na pasta `specs/features/<feature-id>/` correta.3. [ ] Não invade âmbito de outra fase (§3 regras 3 e 4).
