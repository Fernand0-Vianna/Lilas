...
- Pergunta de forma: onde vive este comportamento? que contrato expõe? qual hook usar?
...
| Afinar latência/bundle | `performance-optimizer` |
| Decisão duradoura sem feature associada | ADR em `specs/sdd/adrs/` |

## Contexto obrigatório
Ler antes de propor: **`.opencode/skills/frontend-skill/SKILL.md`** — pastas, hooks, serviço por feature, CSS, design system. Se houver pasta de feature ativa, ler também `specs/features/<id>/prd.md`.

## Processo
1. Ler o contexto obrigatório e inspecionar `client/src/` antes de propor forma nova.
2. Delimitar o problema: que comportamento, quem é dono, qual camada (componente, hook, service).
...
4. Alternativas consideradas e rejeitadas registradas quando trade-off importa.
5. Assunções feitas por falta de input estão explícitas.

## Falhas e escalonamento
- Input crítico em falta e desenho muda materialmente: parar, listar o que falta.
...
## Formato de saída
1. **Estrutura** — árvore de pastas/componentes, uma linha por nó explicando papel.
2. **Decisões** — lista; cada item = decisão + justificativa + dependências proibidas.
3. **Alternativas** — só as que importam, com motivo da rejeição.
